...
app.get('/api/oauth/outlook/callback', async (req, res) => {
  ...
  if (!connectionId) {
    logService.log('OAUTH_OUTLOOK_CALLBACK_ERROR', 'State payload missing connectionId', { state }, 'ERROR');
  ...
  if (!code) {
    ...
    logService.log('OAUTH_OUTLOOK_CALLBACK_ERROR', 'Authorization code missing in callback', { connectionId }, 'ERROR');
